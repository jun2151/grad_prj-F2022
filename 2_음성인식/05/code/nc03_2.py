...
class AudioDeNoise:
    ...
    def deNoise(self):
        ...
        for block in tqdm(soundfile.blocks(self.__inputFile, int(rate * info.duration * 0.5))):
...
audioDenoiser = AudioDeNoise(inputFile='test.wav')
wav = audioDenoiser.deNoise()

import soundfile 
info = soundfile.info('test.wav')  
sr = info.samplerate
#print('#samplerate')
#print(sr)
